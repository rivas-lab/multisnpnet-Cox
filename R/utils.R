...
readPheMaster <- function(phenotype.file, psam.ids, family, covariates, phenotype, status, split.col){
    ...
    CC = list(character=c("FID", "IID"), integer=c(status), numeric=c(covariates, phenotype))
    phe.master.unsorted <- data.table::fread(phenotype.file, colClasses = CC, select = selectCols)
    ...
    phe.master = phe.master[!duplicated(phe.master$ID),]
