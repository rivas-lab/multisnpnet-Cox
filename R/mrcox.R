#' @export
solve_aligned = function(X, y_list, status_list, lambda_1, lambda_2, p.fac=NULL, B0=NULL){
  K = length(y_list)
  N = nrow(X)
  p = ncol(X)
  order_list = list()
  ...
  if(is.null(B0)){
    ...
    if(ncol(B0) > K){
      msg = paste("The number of columns of B0 is greater than K,",
                  "B0[,(K+1):] will be used for proximal step.")
      print(msg)
    }
  }
